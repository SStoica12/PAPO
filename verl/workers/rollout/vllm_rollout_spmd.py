...
class vLLMRollout(BaseRollout):
    def __init__(self, model_path: str, config: RolloutConfig, tokenizer: PreTrainedTokenizer):
        ...
        # add handling of stop tokens and stop words
        if config.stop_token_ids is not None or config.stop_words is not None:
            if config.stop_token_ids is None and config.stop_words is not None:
                stop_token_ids = tokenizer.convert_tokens_to_ids(config.stop_words)
            else:
                stop_token_ids = config.stop_token_ids
            print(f"!INFO: Set stop_token_ids: {stop_token_ids}")
            sampling_kwargs = {
