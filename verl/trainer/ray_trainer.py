...
"""
PAPO Trainer with Ray-based single controller.
...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, 
...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            # if n > 1 we need to repeat the labels
            if self.config.worker.rollout.val_override_config['n'] > 1:
                test_batch = test_batch.repeat(
                    repeat_times=self.config.worker.rollout.val_override_config['n'], 
                    interleave=True
                )
        ...
        self.best_val_score = max(self.best_val_score, reward_score)
    ...
    def _save_best_checkpoint(self) -> None:
        if not self.config.trainer.save_best_checkpoint:
            return
        folder_path = os.path.join(self.config.trainer.save_checkpoint_path, f"val_best_step")
        actor_path = os.path.join(folder_path, "actor")
        self.actor_rollout_wg.save_checkpoint(actor_path)
        ...
        best_global_step_path = os.path.join(self.config.trainer.save_checkpoint_path, "val_best_global_step.txt")
        with open(best_global_step_path, "w") as f:
            f.write(str(self.global_step))
            ...
            for batch_dict in tqdm(self.train_dataloader, desc="Running step", position=1):
                ...
                if self.config.algorithm.use_kl_prcp and "multi_modal_data" in batch.non_tensor_batch.keys():
                    ...
                    for item in batch.non_tensor_batch["multi_modal_data"]:
                        ...
                        aug_multi_modal_data.append({"image": aug_images_pil})
                        aug_multi_modal_inputs.append(dict(aug_multimodal_input))
                    # add to batch
                    batch.non_tensor_batch["aug_multi_modal_data"] = aug_multi_modal_data
                    batch.non_tensor_batch["aug_multi_modal_inputs"] = aug_multi_modal_inputs
                    assert batch.non_tensor_batch["aug_multi_modal_inputs"][0]['pixel_values'].shape == batch.non_tensor_batch["multi_modal_inputs"][0]['pixel_values'].shape
                ...
                with timer("step", timing_raw):
                    ...
                    with timer("gen", timing_raw):  # wg: worker group
                        ...
                        # gen_batch_output.batch['responses'] contains the generated response ids (padded)
                    ...
                    batch.non_tensor_batch.pop("aug_multi_modal_data", None)
                    ...
                    if self.config.algorithm.use_kl_prcp and "aug_multi_modal_inputs" in batch.non_tensor_batch.keys():
                        ...
                        with timer("aug_probs", timing_raw):
                            ...
                            aug_batch.non_tensor_batch["multi_modal_inputs"] = batch.non_tensor_batch.pop("aug_multi_modal_inputs")
                            aug_log_probs = self.actor_rollout_wg.compute_log_probs_aug(aug_batch)
                    ...
                    if (
                        ...
                        # check if its best, if so, save the checkpoint
                        if self.best_val_score == val_metrics["val/reward_score"]:
                            with timer("save_checkpoint", timing_raw):
                                print("saving best checkpoint... val_score: ", self.best_val_score)
